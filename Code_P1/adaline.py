...
def main():
    ...
    if modo_funcionamiento == 3 and len(sys.argv) < 6:
    ...
    adaline = RedNeuronal("adaline", float(sys.argv[3]))
    lista_neuronas_entrada = []

    for i in range(n_atributos_entrada):
        n = NeuronaAdaline("NeuronaEntrada_" +str(i), 0, 0)
    ...
    neurona_salida = NeuronaAdaline("NeuronaSalida", 0, 1)

    # Generamos todos los enlaces, de las neuronas de entrada, a las de salida
    for neurona in lista_neuronas_entrada:
        e = Enlace(random.uniform(-0.5,0.5), neurona, neurona_salida)

        # Añadimos el enlace a la
        adaline.addEnlace(e)
    ...
    if modo_funcionamiento == 1:
        ...
        _, _, datosEntrada_test = leer_fichero(sys.argv[5])
        ...
        nombre_fichero_salida = fichero_entrada_sin_extension[0] +"_salida_adaline.txt"
    ...
    for epoca in range(num_epocas):
        ...
        for i, dato in enumerate(atributos_train):
            ...
            for ind, atributo in enumerate(dato):
                ...
                adaline.enlaces[ind].get_neuronaOrigen().set_valor(atributo)

            # Calcular respuesta de la neurona de salida
            b = neurona_salida.get_sesgo()
            adaline.calculo_suma_pesada()
            y_in = b + neurona_salida.get_suma_pesada_entradas()

            # Calculamos el error cuadratico para posteriormente actualizar los pesos
            ...
            for enlace in adaline.enlaces:
                peso_actual = enlace.get_peso()
                peso_actualizado = peso_actual + (adaline.get_constante_aprendizaje() * error_cuadratico * float(enlace.get_neuronaOrigen().get_valor()))
            ...
            sesgo_actualizado = sesgo_actual + (adaline.get_constante_aprendizaje() * error_cuadratico)
    ...
    # plt.title("Evolucion ECM del Adaline")
    # plt.show()
